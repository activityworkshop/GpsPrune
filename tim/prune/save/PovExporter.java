...
public class PovExporter extends Export3dFunction
{
	...
	public PovExporter(App inApp)
	{
		...
		// Set default camera coordinates and object sizes
		_cameraX = "17";
		_cameraY = "13";
		_cameraZ = "-20";
		_sphereSize = "0.2";
	...
	public void setCameraCoordinates(double inX, double inY, double inZ)
	{
		...
		if (cameraDist > 0.0)
		{
			_cameraX = NumberUtils.formatNumberUk(inX / cameraDist
				* DEFAULT_CAMERA_DISTANCE, 5);
			_cameraY = NumberUtils.formatNumberUk(inY / cameraDist
				* DEFAULT_CAMERA_DISTANCE, 5);
			// Careful! Need to convert from java3d (right-handed) to povray
			// (left-handed) coordinate system!
			_cameraZ = NumberUtils.formatNumberUk(-inZ / cameraDist
				* DEFAULT_CAMERA_DISTANCE, 5);
		}
	}
	...
	public void begin()
	{
		...
		if (_dialog == null)
		{
			_dialog = new JDialog(_parentFrame,
	...
	private Component makeDialogComponents()
	{
		...
		centralPanel.setBorder(panelBorder);

		JLabel fontLabel = new JLabel(
			I18nManager.getText("dialog.exportpov.font"));
		...
		JLabel cameraYLabel = new JLabel(
			I18nManager.getText("dialog.exportpov.cameray"));
		...
		JLabel cameraZLabel = new JLabel(
			I18nManager.getText("dialog.exportpov.cameraz"));
		...
		JLabel altitudeCapLabel = new JLabel(
		...
		centralPanel.add(spheresSizeLabel);
		_sphereSizeField = new JTextField("0.2");
		centralPanel.add(_sphereSizeField);
		...
		centralPanel.add(rodSizeLabel);
		_rodSizeField = new JTextField("0.1");
		centralPanel.add(_rodSizeField);
		...
		_tubesButton.setSelected(true);
		radioPanel.add(_tubesButton);
		...
		// Radio buttons for camera projection type
		...
		boxPanel.add(Box.createVerticalStrut(4));
		boxPanel.add(projectionPanel);
		boxPanel.add(Box.createVerticalStrut(4));
		boxPanel.add(lightingPanel);
		boxPanel.add(Box.createVerticalStrut(4));
	...
	private void doExport()
	{
		...
		_cameraX = checkValue(_cameraXField.getText());
		_cameraY = checkValue(_cameraYField.getText());
		...
		if (_fileChooser == null)
		{
			...
			_fileChooser.setFileFilter(new GenericFileFilter(
				"filetype.pov", new String[] {"pov"}));
			_fileChooser.setAcceptAllFileFilterUsed(false);
			// start from directory in config which should be set
			final String configDir =
				Config.getConfigString(Config.KEY_TRACK_DIR);
			if (configDir != null)
			{
		...
		{
			chooseAgain = false;
			if (_fileChooser.showSaveDialog(_parentFrame)
				== JFileChooser.APPROVE_OPTION)
			{
				...
				final File imageFile = new File(povFile.getParentFile(),
					povFile.getName().substring(0, nameLen) + "_base.png");
				final File terrainFile = new File(povFile.getParentFile(),
					povFile.getName().substring(0, nameLen) + "_terrain.png");
				final boolean imageExists =
					_baseImagePanel.getImageDefinition().getUseImage()
						&& imageFile.exists();
				final boolean terrainFileExists =
					_terrainPanel.getUseTerrain() && terrainFile.exists();

				// Check if files exist and if necessary prompt for overwrite
				Object[] buttonTexts = {I18nManager.getText("button.overwrite"),
					I18nManager.getText("button.cancel")};
				if ((!povFile.exists() && !imageExists && !terrainFileExists)
					|| JOptionPane.showOptionDialog(_parentFrame,
						I18nManager.getText("dialog.save.overwrite.text"),
						I18nManager.getText("dialog.save.overwrite.title"),
					...
					== JOptionPane.YES_OPTION)
				{
					...
					if (exportFiles(povFile, imageFile, terrainFile))
					{
						...
						Config.setConfigString(Config.KEY_TRACK_DIR,
							povFile.getParentFile().getAbsolutePath());
	...
	private boolean exportFiles(
		File inPovFile, File inImageFile, File inTerrainFile)
	{
		...
		try
		{
			...
			// set povray model size, actual always to MODEL_SCALE_FACTOR
			model.setModelSize(MODEL_SCALE_FACTOR);
			try
			{
				// try to use given altitude cap
				double givenFactor =
					Double.parseDouble(_altitudeFactorField.getText());
			...
			if (useImage)
			{
				...
				MapSource mapSource = MapSourceLibrary.getSource(
					imageDef.getSourceIndex());
				MapGrouter grouter = _baseImagePanel.getGrouter();
				GroutedImage baseImage = grouter.getMapImage(
					_track, mapSource, imageDef.getZoom());
				try
				{
					useImage = ImageIO.write(
						baseImage.getImage(), "png", inImageFile);
				}
				catch (IOException ioe) {
					System.err.println(
						"Can't write image: " + ioe.getClass().getName());
					useImage = false;
				}
				if (!useImage)
				{
					_app.showErrorMessage(getNameKey(),
			...
			if (useTerrain)
			{
				TerrainHelper terrainHelper =
					new TerrainHelper(_terrainPanel.getGridSize());

				// See if there's a previously saved terrain track we can reuse
				TerrainDefinition terrainDef =
					new TerrainDefinition(_terrainPanel.getUseTerrain(),
						_terrainPanel.getGridSize());
				Track terrainTrack = TerrainCache.getTerrainTrack(
					_app.getCurrentDataStatus(), terrainDef);
				if (terrainTrack == null)
				{
					// Construct the terrain track according to these extents
					// and the grid size
					...
					{
						try // just polling in a wait loop isn't ideal but simple
						{
							Thread.sleep(750);
						}
					...
					TerrainCache.storeTerrainTrack(
						terrainTrack, _app.getCurrentDataStatus(), terrainDef);
				}
			...
			for (p = 0; p < model.getNumPoints(); p++)
			{
				_scaledLonRange.addValue(model.getScaledHorizValue(p));
				_scaledLatRange.addValue(model.getScaledVertValue(p));
			...
			*/
			...
			else if (_tubesButton.isSelected())
			{
				writeDataPointsTubesAndWalls(writer, model, lineSeparator);
			}
			else
			{
				writeDataPointsSpheres(writer, model, lineSeparator);
			}
			...
			UpdateMessageBroker.informSubscribers(
				I18nManager.getText("confirm.save.ok1")
					+ " " + _track.getNumPoints() + " "
					+ I18nManager.getText("confirm.save.ok2")
		...
		catch (IOException ioe)
		{
			JOptionPane.showMessageDialog(_parentFrame,
				I18nManager.getText("error.save.failed")
					+ " : " + ioe.getMessage(),
	...
	private void writeStartOfFile(FileWriter inWriter, String inLineSeparator,
		File inImageFile, File inTerrainFile, String projection,
		String lighting) throws IOException
	{
		...
		inWriter.write(inLineSeparator);
		inWriter.write("// povray Width=800 Height=600 +V \\");
		inWriter.write(inLineSeparator);
		inWriter.write("// Input_File_Name=test.pov \\");
		inWriter.write(inLineSeparator);
		inWriter.write("// Output_File_Name=test.png \\");
		inWriter.write(inLineSeparator);
		inWriter.write("// Pause_When_Done=true Quality=9 Antialias=true \\");
		inWriter.write(inLineSeparator);
		inWriter.write("// Antialias_Threshold=2");
		inWriter.write(inLineSeparator);
		inWriter.write(inLineSeparator);
		inWriter.write("// Important note :");
		inWriter.write(inLineSeparator);
		inWriter.write("// The camera is set to an aspect ratio of 4/3 by");
		inWriter.write(inLineSeparator);
		inWriter.write("// default. Rendering the image with a different");
		inWriter.write(inLineSeparator);
		inWriter.write("// aspect ratio will distort the output image.");
		inWriter.write(inLineSeparator);
		inWriter.write("// The aspect ratio can be modified with the 'right'");
		inWriter.write(inLineSeparator);
		inWriter.write("// keyword in the camera block. The general way to");
		inWriter.write(inLineSeparator);
		inWriter.write("// set the correct aspect ratio for the image");
		inWriter.write(inLineSeparator);
		inWriter.write("// dimensions is:");
		inWriter.write(inLineSeparator);
		inWriter.write("//     camera");
		inWriter.write(inLineSeparator);
		inWriter.write("//         { right x*ImageWidth/ImageHeight");
		inWriter.write(inLineSeparator);
		inWriter.write("//           (other camera settings...)");
		inWriter.write(inLineSeparator);
		inWriter.write("//         }");
		inWriter.write(inLineSeparator);
		inWriter.write(inLineSeparator);
		...
		// Make the definition of the base plane depending on whether there's
		// an image or not
		...
		final String boxDefinition = (useImageOnBox ?
			"   <0, 0, 0>, <1, 1, 0.001>" + inLineSeparator
				+ "   pigment {image_map { png \"" + inImageFile.getName()
				+ "\" map_type 0 interpolate 2 once } }" + inLineSeparator
		...
		// TODO: Maybe could use the same geometry for the imageless case,
		// would simplify code a bit

		// Definition of terrain shape if any
		final String terrainDefinition = makeTerrainString(
			inTerrainFile, inImageFile, inLineSeparator);
		...
		String[] outputLines = {
		  ...
		  "  sphere {",
		  "   <0, 0, 0>, " + _sphereSize, // size should depend on model size
		  "   texture {",
		...
		// write cardinals
		  ...
		  "text {",
		  "  ttf \"" + fontPath + "\" \""
			  + I18nManager.getText("cardinal.n") + "\" 0.3, 0",
		  "  pigment { color rgb <1 1 1> }",
		  "  translate <0, 0.2, 10.0>",
		  "}",
		  "text {",
		  "  ttf \"" + fontPath + "\" \""
			  + I18nManager.getText("cardinal.s") + "\" 0.3, 0",
		  "  pigment { color rgb <1 1 1> }",
		  "  translate <0, 0.2, -10.0>",
		  "}",
		  "text {",
		  "  ttf \"" + fontPath + "\" \""
			  + I18nManager.getText("cardinal.e") + "\" 0.3, 0",
		  "  pigment { color rgb <1 1 1> }",
		  "  translate <9.7, 0.2, 0>",
		  "}",
		  "text {",
		  "  ttf \"" + fontPath + "\" \""
			  + I18nManager.getText("cardinal.w") + "\" 0.3, 0",
		  "  pigment { color rgb <1 1 1> }",
		...
		String[] standardLights = {
			// standard lighting with three light sources
			// MAYBE: Light positions should relate to model size
			"// lighting preferred for view without height_field",
		...
		String[] cartographicLight = {
			// NW parallel : standard in topographic cartography maps
			"// lighting preferred for view with height_field",
		...
		if (lighting.equals(_stdLighting))
		{
			numLines = standardLights.length;
			for (int i = 0; i < numLines; i++)
			{
				inWriter.write(standardLights[i]);
				inWriter.write(inLineSeparator);
			}
		}
		else
		{
			numLines = cartographicLight.length;
			for (int i = 0; i < numLines; i++)
			{
				inWriter.write(cartographicLight[i]);
				inWriter.write(inLineSeparator);
			}
		}
		...
		{
		numLines = outputScale.length;
		for (int i = 0; i < numLines; i++)
			{
				inWriter.write(outputScale[i]);
				inWriter.write(inLineSeparator);
			}
		}
		...
		{
			numLines = lightGroupTrack.length;
			for (int i = 0; i < numLines; i++)
			{
				inWriter.write(lightGroupTrack[i]);
				inWriter.write(inLineSeparator);
			}
		}
	}
	...
	/**
	 * Make a description of the height_field object for the terrain, depending
	 * on terrain and image.
	 *
	 ...
	 */
	private static String makeTerrainString(
		File inTerrainFile, File inImageFile, String inLineSeparator)
	{
		if (inTerrainFile == null) {return "";}
		StringBuilder sb = new StringBuilder();
		sb.append("//Terrain").append(inLineSeparator)
			.append("height_field {").append(inLineSeparator)
			.append("\tpng \"").append(inTerrainFile.getName())
			.append("\" smooth").append(inLineSeparator)
			.append("\tfinish {diffuse 0.7 phong 0.2}").append(inLineSeparator);
		if (inImageFile != null)
		{
			sb.append("\tpigment {image_map { png \"")
				.append(inImageFile.getName()).append("\"  } rotate x*90}")
				.append(inLineSeparator);
		}
		else
		{
			sb.append("\tpigment {color rgb <0.55 0.7 0.55> }")
			.append(inLineSeparator);
		}
		sb.append("\tscale 20.0").append(inLineSeparator)
			.append("\ttranslate <-10.0, 0, -10.0>").append(inLineSeparator)
			.append("}");
		return sb.toString();
	}
	 ...
	 * @throws IOException on file writing error
	 */
	private static void writeDataPointsBallsAndSticks(
		FileWriter inWriter, ThreeDModel inModel, String inLineSeparator)
		throws IOException
	{
		...
		for (int i=0; i<numPoints; i++)
		{
			...
			{
				// waypoint ball
				inWriter.write("object { waypoint_sphere translate <"
					+ inModel.getScaledHorizValue(i)
			...
			else
			{
				...
				inWriter.write("object { track_sphere"
					+ checkHeightCode(inModel.getPointHeightCode(i))
					+ " translate <" + inModel.getScaledHorizValue(i) + ","
					+ inModel.getScaledAltValue(i)
					+ "," + inModel.getScaledVertValue(i) + "> }");
			}
			inWriter.write(inLineSeparator);
			...
			{
				inWriter.write("object { point_rod translate <"
					+ inModel.getScaledHorizValue(i) + ",0,"
					+ inModel.getScaledVertValue(i) + "> scale <1,"
					+ inModel.getScaledAltValue(i) + ",1> }");
				inWriter.write(inLineSeparator);
			}
		}
		inWriter.write(inLineSeparator);
	 ...
	 * @throws IOException on file writing error
	 */
	private static void writeDataPointsSpheres(
		FileWriter inWriter, ThreeDModel inModel, String inLineSeparator)
		throws IOException
	{
		inWriter.write("// Data points:");
		...
		while (segmentIterator.hasNext())
		{
			...
			if (segLength <= 1)
			{
				...
				inWriter.write("object { track_sphere_t"
					+ " translate <" + inModel.getScaledHorizValue(index) + ","
					+ inModel.getScaledAltValue(index)
	 ...
	 * @throws IOException on file writing error
	 */
	private static void writeSphereSweep(FileWriter inWriter,
		...
		throws IOException
	{
		...
		inWriter.write(inLineSeparator);
		String splineType = inSegment.getNumTrackPoints() < 5 ?
		...
		for (int i=inSegment.getStartIndex(); i<=inSegment.getEndIndex(); i++)
		{
			if (inModel.getPointType(i) != ThreeDModel.POINT_TYPE_WAYPOINT)
			{
				inWriter.write("  <" + inModel.getScaledHorizValue(i) + ","
					+ inModel.getScaledAltValue(i)
					+ "," + inModel.getScaledVertValue(i) + ">, " + _rodSize);
				inWriter.write(inLineSeparator);
			}
		}
		inWriter.write("  tolerance 0.1");
		inWriter.write(inLineSeparator);
		inWriter.write("  texture { pigment {color rgb <0.6 1.0 0.2>}  "
			+ "finish {phong 1} }");
		inWriter.write(inLineSeparator);
	 ...
	 * @throws IOException on file writing error
	 */
	private static void writePolygonWall(FileWriter inWriter,
		ThreeDModel inModel, ModelSegment inSegment, String inLineSeparator)
			throws IOException
	{
		// wall
		...
		for (int i=inSegment.getStartIndex(); i<=inSegment.getEndIndex(); i++)
		{
			if (inModel.getPointType(i) != ThreeDModel.POINT_TYPE_WAYPOINT)
			{
				if (prevIndex >= 0)
				{
					double xDiff = inModel.getScaledHorizValue(i)
						- inModel.getScaledHorizValue(prevIndex);
					double yDiff = inModel.getScaledVertValue(i)
	 ...
	 */
	private static String checkValue(String inString)
	{
		double value = 0.0;
		try
		{
	...
	/**
	 * Go through the points making a list of the segment starts and the number
	 * of track points in each segment.
	 *
	...
	private static ArrayList<ModelSegment> getSegmentList(ThreeDModel inModel)
	{
		...
		if (inModel != null && inModel.getNumPoints() > 0)
		{
			...
			for (int i=0; i<inModel.getNumPoints(); i++)
			{
				if (inModel.getPointType(i) != ThreeDModel.POINT_TYPE_WAYPOINT)
				{
					if (inModel.getPointType(i)
						== ThreeDModel.POINT_TYPE_SEGMENT_START
						|| currSegment == null)
					{
						// start of segment
						if (currSegment != null)
